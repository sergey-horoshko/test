<template>
  <div class="rounded-lg p-5 md:bg-white md:p-12 md:shadow-xl dark:md:bg-dark-content">
    <div class="flex w-full items-center justify-between gap-3">
      ...
      <router-link class="font-semibold text-blue-600 dark:text-blue-500" to="/registration">
        {{ $t('auth.registration') }}
      </router-link>
    </div>
    <div class="mt-1">
    ...
    <form @submit.prevent="onSubmit" class="mt-8">
      ...
      <div class="mt-1 text-right">
        <router-link class="text-xs font-medium text-blue-600 dark:text-blue-500" to="/reset">
          {{ $t('auth.reset') }}
        </router-link>
      </div>
      ...
      <Button
        ...
        :label="form.pending ? '' : $t('form.buttonSignIn')"
        rounded
        type="submit"
      />
    </form>
...
import { AuthRequest } from '@/api/login.js';
